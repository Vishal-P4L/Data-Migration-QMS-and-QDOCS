...
if source_file and target_file:
    ...
    if "Transformation Logic" in df.columns:
        ...
        if not approved.empty:
            ...
            def parse_date(row):
                if pd.isna(row) or row == '':  # Check if the date is NaN or an empty string
                    return pd.NaT  # Return NaT for missing/empty dates
                for fmt in date_formats:
                    try:
                        return pd.to_datetime(row, format=fmt)
                    except ValueError:
                        continue
                return pd.NaT 

            # Apply parsing to 'approved_date__vs'
            source_df['approved_date__vs'] = source_df['approved_date__vs'].apply(parse_date)
    ...
    if picklist_file is not None:
        ...
        choices2 = ['Obsolete', 'Obsolete', 'Superseded', 'Effective', 'Effective', 'Effective', 'Effective']
        final_output_df['status__v'] = np.select(conditions2, choices2, default='')

         # Use np.select and pad or truncate the result
        result = np.select(conditions2, choices2, default='')

        # Ensure the result matches the length of the DataFrame
        result = result[:len(final_output_df)]  # Truncate if it's longer
        result = np.pad(result, (0, len(final_output_df) - len(result)), constant_values='')  # Pad if it's shorter

        # Assign the result to the DataFrame column
        final_output_df['status__v'] = result
